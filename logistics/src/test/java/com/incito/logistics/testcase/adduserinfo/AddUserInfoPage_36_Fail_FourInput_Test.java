...
/**
 * @author xy-incito-wk
 * @Description 填写四个文本框点击提交
 *
 * */
public class AddUserInfoPage_36_Fail_FourInput_Test extends BaseParpare {
	@Test
	public void addUserInfoPageFailTelIdcardCompanyAddress(ITestContext context) {
		...
		// 循环填写一个文本框并检测提示语
		for (int l = 0; l < 5; l++) {
			System.out.println("第" + l + "层循环！！！");
			for (int k = l + 1; k < 5; k++) {
				System.out.println("第" + l + "层中的，第" + k + "个循环，开始！！！");
				for (int j = k + 1; j < 5; j++) {
					System.out.println("第" + l + "层中的，第" + k + "个循环，第" + j + "个循环开始了！！！");
					for (int i = j + 1; i < 5; i++) {
						System.out.println("第" + l + "层中的，第" + k + "个循环，第" + j + "个循环，第" + i + "个循环开始填写了！！！");
						seleniumUtil.type(seleniumUtil.findElementBy(bys[l]), add_text[l]);
